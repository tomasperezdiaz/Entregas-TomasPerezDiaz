...
const renderizar = (msgs) => {
  ...
  msgs.forEach((message) => {
    ...
    const messageClass = isCurrent ? "my-message" : "other-message";
    messages =
      messages +
...
Swal.fire({
  ...
  inputValidator: (value) => {
    if (!value) return "Necesitas un correo electronico para ingresar";

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(value)) return "Ingrese un correo valido";
    return null;
  },
